...
export default function IslamicToolsOverlay(){
 ...
 useEffect(()=>{const a=()=>setOffline(false),b=()=>setOffline(true);addEventListener("online",a);addEventListener("offline",b);return()=>{removeEventListener("online",a);removeEventListener("offline",b)}},[]);
 ...
 useEffect(()=>{
   const openFromApp=(event:Event)=>{
     const requested=(event as CustomEvent<string>).detail as Tool;
     if(["qibla","ramadan","calendar","duas","hadith","tasbeeh","notifications","settings"].includes(requested)){
       setTool(requested);setOpen(true);
     }
   };
   const clickFromApp=(event:MouseEvent)=>{
     const target=event.target as HTMLElement|null;
     const button=target?.closest("button") as HTMLButtonElement|null;
     if(!button)return;
     const container=button.closest(".quick-grid, .more-list");
     if(!container)return;
     const label=button.textContent?.trim().toLowerCase()||"";
     const map:Record<string,Tool>={hadith:"hadith",duas:"duas",qibla:"qibla",tasbeeh:"tasbeeh",calendar:"calendar",ramadan:"ramadan",settings:"settings"};
     const requested=Object.keys(map).find(key=>label.includes(key));
     if(requested){event.preventDefault();event.stopPropagation();setTool(map[requested]);setOpen(true);}
   };
   addEventListener("muslim-guide:open-tool",openFromApp as EventListener);
   document.addEventListener("click",clickFromApp,true);
   return()=>{removeEventListener("muslim-guide:open-tool",openFromApp as EventListener);document.removeEventListener("click",clickFromApp,true)};
 },[]);
 const today=useMemo(()=>{const d=new Date(),h=hijri(d);return`${d.toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long"})} · ${h.day} ${h.month} ${h.year}`},[]);
 ...
 const titles:Record<string,string>={qibla:"Qibla Finder",ramadan:"Ramadan",calendar:"Islamic Calendar",duas:"Duas & Azkar",hadith:"Hadith",tasbeeh:"Tasbeeh",notifications:"Notifications",settings:"Settings"};
 const cards:any[]=[["qibla","Qibla","Live compass & direction",Compass],["ramadan","Ramadan","30-day fasting planner",Moon],["calendar","Islamic Calendar","Hijri dates & events",CalendarDays],["duas","Duas & Azkar","Authentic references",Heart],["hadith","Hadith","Six major collections",BookOpenIcon],["tasbeeh","Tasbeeh","Digital counter",CircleDot],["notifications","Notifications","Daily prayer reminders",Bell],["settings","Settings","Prayer preferences",Settings]];
