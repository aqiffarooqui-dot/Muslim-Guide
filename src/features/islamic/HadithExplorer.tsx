...
import { Bookmark, BookmarkCheck, Copy, ExternalLink, Loader2, Search, Share2 } from "lucide-react";
...
function collectionName(id: CollectionId) {
...
function parseHadithPayload(payload: unknown): Hadith[] {
  ...
  if (payload && typeof payload === "object") {
    ...
    for (const key of ["hadiths", "data", "items", "results"]) {
      if (Array.isArray(value[key])) return value[key] as Hadith[];
    }
...
export default function HadithExplorer() {
  ...
  const [bookmarks, setBookmarks] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem("muslim-guide-hadith-bookmarks") || "[]"); } catch { return []; }
  });
  ...
  useEffect(() => {
    ...
    setError("");
    const cacheKey = `muslim-guide-hadith-tree-${collection}`;
    const load = async () => {
      try {
        let tree: TreeItem[] = [];
        const cached = localStorage.getItem(cacheKey);
        if (cached) tree = JSON.parse(cached);
        if (!tree.length) {
          const response = await fetch(SOURCE_API, { headers: { Accept: "application/vnd.github+json" } });
        ...
        if (!cancelled) setError(e instanceof Error ? e.message : "Could not load Hadith books.");
      } finally {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await fetch(`${SOURCE_BASE}${book}`);
        if (!response.ok) throw new Error(`Hadith book unavailable (${response.status})`);
        const payload = await response.json();
        if (!cancelled) setHadiths(parseHadithPayload(payload));
      } catch (e) {
        if (!cancelled) setError(e instanceof Error ? e.message : "Could not load this Hadith book.");
      } finally {
        if (!cancelled) setLoadingHadiths(false);
      }
    };
    load();
    return () => { cancelled = true; };
  ...
  useEffect(() => {
    try { localStorage.setItem("muslim-guide-hadith-bookmarks", JSON.stringify(bookmarks)); } catch { /* storage may be unavailable */ }
  }, [bookmarks]);

  const visible = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return hadiths.slice(0, 40);
    return hadiths.filter(h => [h.english, h.arabic, h.reference, h.grade, h.book].some(v => clean(v).toLowerCase().includes(q))).slice(0, 80);
  }, [hadiths, query]);

  const toggleBookmark = (h: Hadith, index: number) => {
    const key = `${collection}:${book}:${h.id ?? index}`;
  ...
  const copyHadith = async (h: Hadith) => {
    const text = [h.arabic, h.english, h.reference, h.grade].filter(Boolean).join("\n\n");
    try { await navigator.clipboard.writeText(text); } catch { /* clipboard may be blocked */ }
  };

  const shareHadith = async (h: Hadith) => {
    const text = [h.english || h.arabic, h.reference].filter(Boolean).join("\n\n");
    if (navigator.share) await navigator.share({ title: `${collectionName(collection)} Hadith`, text });
    else await copyHadith(h);
  };
  ...
  return <div className="tool-page hadith-explorer">
    ...
    <div className="hadith-collections">
      {collections.map(c => <button key={c.id} className={`hadith-collection ${collection === c.id ? "active" : ""}`} onClick={() => { setCollection(c.id); setQuery(""); }}><strong>{c.name}</strong><span>{c.description}</span></button>)}
    </div>
    <div className="hadith-controls">
      <select value={book} onChange={e => setBook(e.target.value)} disabled={loadingBooks || !books.length} aria-label="Hadith book">
        {books.map(item => <option key={item.path} value={item.path}>{item.path.split("/").pop()?.replace(/\.json$/, "").replace(/^\d+_/, "").replace(/_/g, " ")}</option>)}
      </select>
      <div className="hadith-search"><Search size={18}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search this book..." /></div>
    </div>
    ...
    {!loadingHadiths && !error && <div className="hadith-list">
      {visible.map((h, index) => <article className="content-card hadith-card" key={`${h.id ?? index}-${h.reference ?? "hadith"}`}>
        <div className="hadith-card-top"><span>{h.reference || `${collectionName(collection)} · Hadith ${h.id ?? index + 1}`}</span><div className="hadith-actions"><button onClick={() => toggleBookmark(h, index)} aria-label="Bookmark Hadith">{isBookmarked(h, index) ? <BookmarkCheck size={18}/> : <Bookmark size={18}/>}</button><button onClick={() => copyHadith(h)} aria-label="Copy Hadith"><Copy size={18}/></button><button onClick={() => shareHadith(h)} aria-label="Share Hadith"><Share2 size={18}/></button></div></div>
        {h.arabic && <p className="arabic-tool hadith-arabic" dir="rtl">{clean(h.arabic)}</p>}
        {h.english && <p className="hadith-english">{clean(h.english)}</p>}
        {h.grade && <small className="hadith-grade">{clean(h.grade)}</small>}
      </article>)}
      {!visible.length && <div className="tool-note">No Hadith matched your search in this book.</div>}
      {visible.length > 0 && !query && hadiths.length > visible.length && <div className="tool-note">Showing the first {visible.length} Hadiths. Use search to find a specific narration.</div>}
    </div>}
